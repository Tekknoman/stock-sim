        ...
        <div className="container mx-auto">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <span>{tradingSidebarOpen ? 'Hide Trading Panel' : 'Show Trading Panel'}</span>
          ...
          {tradingSidebarOpen && (
            <div className="w-80 bg-dark-400 rounded-lg shadow-lg overflow-hidden flex flex-col">
              ...
              <div className="flex border-b border-dark-200">
                <button 
                  className={`flex-1 py-3 font-medium text-center transition-colors ${activeTab === 'buy' ? 'bg-dark-300 text-primary-300' : 'hover:bg-dark-300/50'}`}
                  onClick={() => setActiveTab('buy')}
                >
                  Buy Stocks
                </button>
                <button 
                  className={`flex-1 py-3 font-medium text-center transition-colors ${activeTab === 'positions' ? 'bg-dark-300 text-primary-300' : 'hover:bg-dark-300/50'}`}
              ...
              <div className="flex-1 overflow-auto p-4">
                {activeTab === 'buy' && (
                  <PositionForm onComplete={(position) => {
                    // Refresh user positions and add to transaction history
                    if (selectedUser) {
                      selectUser(selectedUser);
                      addTransaction(`Bought ${position.amount} shares of ${position.stock_name} at $${position.open_price.toFixed(2)}`, 'buy');
                    }
                  }} />
                )}
                  ...
                  <>
                    ...
                    <PositionList 
                      ...
                      onSellPosition={(position, price) => {
                        addTransaction(
                          `Sold ${position.amount} shares of ${position.stock_name} at $${price.toFixed(2)}`,
                          'sell'
                        );
                      }} 
                    />
                ...
                {activeTab === 'positions' && !selectedUser && (
                  <div className="bg-dark-300 p-6 rounded text-center">
                    <p className="text-gray-400">Select a trader to view positions</p>
                  </div>
                )}
              </div>
                    ...
                    transactionHistory.map((transaction, index) => (
                      <div 
                        ...
                        className={`py-1 px-2 rounded ${
                          transaction.type === 'buy' ? 'bg-blue-900/20' : 'bg-green-900/20'
                        }`}
                      >
